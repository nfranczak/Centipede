# make sure that the time measurement is accurate
# make sure that normalize isnt trunkating the sound produced
...
def call ():
    amplitude = record_to_file('demo.wav')
    with contextlib.closing(wave.open('demo.wav','r')) as f:
        frames = f.getnframes()
        rate = f.getframerate()
